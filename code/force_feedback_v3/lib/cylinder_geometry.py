...
对外接口:
    sample_intersection(cyl1, cyl2, n_samples, N_curve) → Geom
    resample_curve(geom, n_samples) → Geom

数据类:
    Geom — 采样结果 + 两个 CylinderDef
"""
...
def _get_branch_meta(cyl1, cyl2, N):
...
def sample_intersection(
    ...
    meta = _get_branch_meta(cyl1, cyl2, N_curve)
    curves = _intersect_raw(meta)
...
def resample_curve(geom: Geom, n_samples: int) -> Geom:
